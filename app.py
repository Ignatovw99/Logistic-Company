...
class Shipment(db.Model):
    ...
    def __init__(self, weight, from_address,to_address, sender,receiver,price, acceptor=None, deliverer=None,status=ShippingStatus.WAITING):
        self.weight = weight
        self.status = status
...
def ClientShipments():
    ...
    return render_template("client-shipments.html", shipments_client = shipments_client_data, offices = office_data, statuses = status_list)
...
def shipments_status():
    ...
    return render_template("shipping.html", shipments = status_shipments, statuses = status_list)

...
@delivery.route("/dispatch/<shipment_id>")
@login_required
@role_required(Role.EMPLOYEE)
...
def transport(shipment_id):
    ...
    if not shipment:
        ...
        flash("Only the courier who dispatched the shipment can transport it")
    else:
...
# TODO: refactor the last both handlers if needed
@delivery.route("/ship/<shipment_id>")
@login_required
@role_required(Role.EMPLOYEE)
def ship_to_customer(shipment_id):
    current_employee = find_employee_by_id(current_user.id)
    shipment = find_shipment_by_id(shipment_id)
    ...
    elif current_employee.is_courier():
...
def deliver_shipment(shipment_id):
    ...
    elif shipment.status == ShippingStatus.TRAVELING_TO_YOUR_ADDRESS and not current_employee.is_courier():
        flash("Shipment in status TRAVELING_TO_YOUR_ADDRESS can be delivered only by a courier")
    else:

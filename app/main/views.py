...
def download_report_shipments():
    ...
    for row in result:
        ...
        pdf.ln(th)
        pdf.cell(col_width, th, str(row.date))
        pdf.ln(th)
...
def download_report():
    status_list = [ShippingStatus.ACCEPTED, ShippingStatus.READY_TO_PACK, ShippingStatus.READY_TO_SHIP,ShippingStatus.ON_ITS_WAY,
                   ShippingStatus.ARRIVED, ShippingStatus.TRAVELLING_TO_YOUR_ADDRESS,ShippingStatus.DELIVERED]
...
def download_report_shipments_by_status():
    ...
    if status_form == str(ShippingStatus.ACCEPTED):
        ...
        status = ShippingStatus.TRAVELLING_TO_YOUR_ADDRESS
...
@app.route('/download/report/company-revenue/pdf', methods=['GET', 'POST'])
